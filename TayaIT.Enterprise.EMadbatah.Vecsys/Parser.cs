    ...
    public class VecsysParser
    {
        public static TransFile ParseTransXml(string filePath)
        {
            ...
            foreach (XmlNode node in speakersNodes)
            {
                ...
                spk.lang = node.Attributes["lang"] != null ? node.Attributes["lang"].Value : "";
                spk.lconf = node.Attributes["lconf"] != null ? double.Parse(node.Attributes["lconf"].Value) : 0;
                spk.nw = node.Attributes["nw"] != null ? double.Parse(node.Attributes["nw"].Value) : 0;
                spk.spkid = node.Attributes["spkid"].Value;
                spk.tconf = node.Attributes["tconf"] != null ? double.Parse(node.Attributes["tconf"].Value) : 0;
