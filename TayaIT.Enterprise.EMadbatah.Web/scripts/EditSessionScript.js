    ...
    function loadVotesMembers() {
        ...
        jQuery.ajax({
            ...
            success: function (response) {
                ...
                radio = $('<span>').html(vote_types[0])
                th_header_container.append($('<span style="margin: 0 2px;display: inline-block;width: 50px;color:green">').html(vote_types[0])).append($('<span style="margin: 0 2px;display: inline-block;width: 80px;color:red">').html(vote_types[1])).append($('<span style="margin: 0 2px;display: inline-block;color:blue">>').html(vote_types[2]));
                row = $('<tr>').attr({ });
                th1 = $('<th>').attr({}).append($('<span>').html("ID").addClass("displaynone"));
                th2 = $('<th>').attr({}).append($('<span>').html("اسم المتحدث"));
                th3 = $('<th>').attr({}).append($('<span>').html(th_header_container));
                ...
                for (var i = 0; i < response.length; i++) {
                    row = $('<tr>').attr({});
                    td1 = $('<td>').attr({}).html($('<span>').html(response[i].AttendantID).addClass("displaynone"));
                    td2 = $('<td>').attr({}).html($('<span>').html(response[i].AttendantName));
                    td_options = table_options.clone().html().replace(/toBeBinded/g, "options_" + response[i].AttendantID.toString());
                    td3 = $('<td>').attr({}).html(table_options.clone().html(td_options));
